	...
	dispatch : function(handler) {
		for(var i = 0; i < this.listeners.length; i++) {
			try {
				...
				log4Log.warn("Could not run the listener " + this.listeners[i] + ". \n" + e);
			}
		}
	},
...
Log4js.AjaxAppender.prototype = Log4js.extend(new Log4js.Appender(), {  
	 ...
	 */
	doAppend: function(loggingEvent) {
		log4Log.trace("> AjaxAppender.append");
	...
	doClear: function() {
		log4Log.trace("> AjaxAppender.doClear" );
	...
	setThreshold: function(threshold) {
		log4Log.trace("> AjaxAppender.setThreshold: " + threshold );
		this.threshold = threshold;
		log4Log.trace("< AjaxAppender.setThreshold" );
	},
	...
	send: function() {
		if(this.loggingEventMap.length() >0) {
			...
			log4Log.trace("> AjaxAppender.send");
			...
			try {
				window.setTimeout(function(){
					log4Log.trace("> AjaxAppender.timeout");
	...
	onReadyStateChanged: function() {
		log4Log.trace("> AjaxAppender.onReadyStateChanged");
		var req = this.httpRequest;
		if (this.httpRequest.readyState != 4) { 
			log4Log.trace("< AjaxAppender.onReadyStateChanged: readyState " + req.readyState + " != 4");
			return; 
		}
		...
		if (success) {
			log4Log.trace("  AjaxAppender.onReadyStateChanged: success");
		...
		log4Log.trace("< AjaxAppender.onReadyStateChanged: readyState == 4");
	},
	/**
	...
	getXmlHttpRequest: function() {
		log4Log.trace("> AjaxAppender.getXmlHttpRequest");
		...
		if (!httpRequest) {
			log4Log.fatal("Unfortunatelly your browser does not support AjaxAppender for log4js!");
		}
...
Log4js.BrowserConsoleAppender = function() {
	...
	if (window.console) {
       ...
       //@todo
       log4Log.error("Unsupported Browser");
    }
};
